...
void OnLanePlanning::RunOnce(const LocalView& local_view, ADCTrajectory* const ptr_trajectory_pb) 
{
  ...
  else 
  {
    ...
    {
      planning_smoother_.Smooth(injector_->frame_history(), 
                                frame_.get(), 
                                ptr_trajectory_pb );
    }
  }
    ...
    ADCTrajectory* const ptr_trajectory_pb ) 
{
  ...
  if (FLAGS_enable_record_debug) 
  {
    ptr_debug->mutable_planning_data()->mutable_init_point(
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) 
  {
    ...
    if (injector_->vehicle_state()->vehicle_state().driving_mode() != Chassis::DrivingMode::Chassis_DrivingMode_COMPLETE_AUTO_DRIVE) 
    {
    ...
    if (FLAGS_enable_record_debug) 
    {
      ...
      ExportOpenSpaceChart(ptr_trajectory_pb->debug(), 
                           *ptr_trajectory_pb,
                           ptr_debug );
    }
  } 
...
void AddSTGraph(const STGraphDebug& st_graph, Chart* chart) 
{
  ...
  PopulateChartOptions(-2.0, 10.0, "t (second)", 
                       -10.0, 220.0, "s (meter)",
                       false, chart );
...
VehicleState OnLanePlanning::AlignTimeStamp(const VehicleState& vehicle_state, const double curr_timestamp) 
const {
  // TODO(Jinyun): use the same method in trajectory stitching
  //               for forward prediction
  auto future_xy = injector_->vehicle_state(
      )->EstimateFuturePosition(curr_timestamp - vehicle_state.timestamp()) ;
