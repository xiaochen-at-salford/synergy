...
Stream *create_stream(const config::Stream &sd) 
{
  switch (sd.type_case()) 
  {
    ...
    case config::Stream::kNtrip:
      ...
      return Stream::create_ntrip(sd.ntrip().address(), 
                                  static_cast<uint16_t>(sd.ntrip().port()), 
                                  sd.ntrip().mount_point(), 
                                  sd.ntrip().user(), 
                                  sd.ntrip().password(), 
                                  sd.ntrip().timeout_s() );
    default:
...
bool RawStream::Init() 
{
  ...
  gpsbin_stream_.reset(new std::ofstream(gpsbin_file, std::ios::app | std::ios::out | std::ios::binary));
  stream_writer_ = node_->CreateWriter<StreamStatus>(FLAGS_stream_status_topic);
