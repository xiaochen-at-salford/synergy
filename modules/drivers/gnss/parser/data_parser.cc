...
{
  // return Parser::CreateAn(config);
  AINFO << "the an_gnss config is: " << config.data().format();
  switch (config.data().format()) 
  {
    case config::Stream::NOVATEL_BINARY:
      return Parser::CreateNovatel(config);

    //TODO(xiaochen-at-salford): Add the an_gnss case
    case config::Stream::AN_BINARY:
      return Parser::CreateAn(config);
...
void DataParser::DispatchMessage(Parser::MessageType type, MessagePtr message) 
{
  switch (type) 
  {
    ...
    case Parser::MessageType::AN_SYSTEM_STATE:
      PublishAnSystemState(message);
      break;
...
void DataParser::PublishOdometry(const MessagePtr message) 
{
  ...
  // 2. orientation
  Eigen::Quaterniond q = Eigen::AngleAxisd(ins->euler_angles().z() - 90 * DEG_TO_RAD_LOCAL, 
                                           Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(-ins->euler_angles().y(), 
                                           Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(ins->euler_angles().x(), 
...
//TODO()xiaochen-at-salford):
void DataParser::PublishAnSystemState(const MessagePtr message) 
{
