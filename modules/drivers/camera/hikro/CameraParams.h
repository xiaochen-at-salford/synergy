...
#define MV_UNKNOW_DEVICE        0x00000000          // ch:δ֪�豸���ͣ��������� | en:Unknown Device Type, Reserved 
...
#define MV_CAMERALINK_DEVICE    0x00000008          // ch:CameraLink�豸 | en:CameraLink Device
...
typedef struct _MV_CC_DEVICE_INFO_ {
    ...
    unsigned int        nTLayerType;                // ch:�豸�����Э�����ͣ�e.g. MV_GIGE_DEVICE | en:Device Transport Layer Protocol Type, e.g. MV_GIGE_DEVICE
...
{
    int64_t         nReviceDataSize;    // ch:�ѽ������ݴ�С  [ͳ��StartGrabbing��StopGrabbing֮���������] | en:Received Data Size  [Calculate the Data Size between StartGrabbing and StopGrabbing]
...
// ch:���֧�ֵĴ����ʵ������ | en:The maximum number of supported transport layer instances
...
typedef struct _MV_CC_DEVICE_INFO_LIST_
{
    ...
    MV_CC_DEVICE_INFO*      pDeviceInfo[MV_MAX_DEVICE_NUM];     // ch:֧�����256���豸 | en:Support up to 256 devices
...
// ch:ͼ��ṹ�壬���ͼ��ָ���ַ��ͼ����Ϣ | en:Image Struct, output the pointer of Image and the information of the specific image
typedef struct _MV_FRAME_OUT_
{
    unsigned char* pBufAddr;              // ch:ͼ��ָ���ַ | en: pointer of image
    MV_FRAME_OUT_INFO_EX stFrameInfo;     // ch:ͼ����Ϣ | en:information of the specific image
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_
{
    ...
    enum MvGvspPixelType       enPixelType;        // [IN]     ch:�������ݵ����ظ�ʽ | en:Input Data Pixel Format
    ...
    unsigned int          nBufferSize;        // [IN]     ch:�ṩ�������������С | en:Output buffer size provided
    enum MV_SAVE_IAMGE_TYPE    enImageType;        // [IN]     ch:���ͼƬ��ʽ | en:Output Image Format
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_EX_
{
    ...
    unsigned int        nBufferSize;        // [IN]     ch:�ṩ�������������С | en:Output buffer size provided
    enum MV_SAVE_IAMGE_TYPE  enImageType;        // [IN]     ch:���ͼƬ��ʽ | en:Output Image Format
    unsigned int        nJpgQuality;        // [IN]     ch:��������, (50-99] | en:Encoding quality, (50-99]
...
typedef struct _MV_PIXEL_CONVERT_PARAM_T_
{
    ...
    enum MvGvspPixelType    enSrcPixelType;     // [IN]     ch:Դ���ظ�ʽ | en:Source pixel format
    unsigned char*      pSrcData;           // [IN]     ch:�������ݻ��� | en:Input data buffer
    unsigned int        nSrcDataLen;        // [IN]     ch:�������ݴ�С | en:Input data size

    enum MvGvspPixelType    enDstPixelType;     // [IN]     ch:Ŀ�����ظ�ʽ | en:Destination pixel format
    unsigned char*      pDstBuffer;         // [OUT]    ch:������ݻ��� | en:Output data buffer
    unsigned int        nDstLen;            // [OUT]    ch:������ݴ�С | en:Output data size
    unsigned int        nDstBufferSize;     // [IN]     ch:�ṩ�������������С | en:Provided outbut buffer size
    unsigned int        nRes[4];
...
typedef enum _MV_GIGE_TRANSMISSION_TYPE_
{
    ...
    MV_GIGE_TRANSTYPE_LIMITEDBROADCAST       = 0x2,                // ch:��ʾ�������ڹ㲥���ݲ�֧�� | en:Limited broadcast mode,not support
    MV_GIGE_TRANSTYPE_SUBNETBROADCAST        = 0x3,                // ch:��ʾ�����ڹ㲥���ݲ�֧�� | en:Subnet broadcast mode,not support
    MV_GIGE_TRANSTYPE_CAMERADEFINED          = 0x4,                // ch:��ʾ�������ȡ���ݲ�֧�� | en:Transtype from camera,not support
    MV_GIGE_TRANSTYPE_UNICAST_DEFINED_PORT   = 0x5,                // ch:��ʾ�û��Զ���Ӧ�ö˽���ͼ������Port�� | en:User Defined Receive Data Port
    MV_GIGE_TRANSTYPE_UNICAST_WITHOUT_RECV   = 0x00010000,         // ch:��ʾ�����˵���������ʵ��������ͼ������ | en:Unicast without receive data
    MV_GIGE_TRANSTYPE_MULTICAST_WITHOUT_RECV = 0x00010001,         // ch:��ʾ�鲥ģʽ������ʵ��������ͼ������ | en:Multicast without receive data
...
#define MV_MATCH_TYPE_NET_DETECT             0x00000001      // ch:���������Ͷ�����Ϣ | en:Network traffic and packet loss information
#define MV_MATCH_TYPE_USB_DETECT             0x00000002      // ch:host���յ�����U3V�豸���ֽ����� | en:The total number of bytes host received from U3V device


// ch:ĳ���ڵ��Ӧ���ӽڵ�������ֵ | en:The maximum number of child nodes corresponding to a node
...
// ch:ÿ���Ѿ�ʵ�ֵ�Ԫ�����Ƴ��� | en:The length of the name of each unit that has been implemented
...
{
    unsigned int    nType;          // ch:��Ҫ�������Ϣ���ͣ�e.g. MV_MATCH_TYPE_NET_DETECT | en:Information type need to output ,e.g. MV_MATCH_TYPE_NET_DETECT
    void*           pInfo;          // ch:�������Ϣ���棬�ɵ����߷��� | en:Output information cache, which is allocated by the caller
...
{
    int64_t         nReviceDataSize;    // ch:�ѽ������ݴ�С  [ͳ��StartGrabbing��StopGrabbing֮���������] | en:Received data size 
...
// ch:host�յ���u3v�豸�˵����ֽ�������Ӧ����Ϊ MV_MATCH_TYPE_USB_DETECT | en:The total number of bytes host received from the u3v device side, the corresponding type is MV_MATCH_TYPE_USB_DETECT
typedef struct _MV_MATCH_INFO_USB_DETECT_
{
    int64_t         nReviceDataSize;      // ch:�ѽ������ݴ�С    [ͳ��OpenDevicce��CloseDevice֮���������] | en:Received data size
    unsigned int    nRevicedFrameCount;   // ch:���յ���֡�� | en:Number of frames received
...
typedef struct _MV_IMAGE_BASIC_INFO_
{
    ...
    unsigned int        nSupportedPixelFmtNum;      // ch:֧�ֵ����ظ�ʽ���� | en:Support pixel format
...
#define MV_EXCEPTION_DEV_DISCONNECT     0x00008001      // ch:�豸�Ͽ����� | en:The device is disconnected
#define MV_EXCEPTION_VERSION_CHECK      0x00008002      // ch:SDK�������汾��ƥ�� | en:SDK does not match the driver version
...
#define MAX_EVENT_NAME_SIZE     128//���Event�¼�������󳤶� | en:Max length of event name
...
typedef struct _MV_XML_NODE_FEATURE_
{
    ...
    char                    strDescription[MV_MAX_XML_DISC_STRLEN_C];// ch:�ڵ�����     Ŀǰ�ݲ�֧�� | en:Node Description, NOT SUPPORT NOW
...
typedef struct _MV_XML_FEATURE_Integer_
{
    ...
    char                strDescription[MV_MAX_XML_DISC_STRLEN_C];   // ch:Ŀǰ�ݲ�֧�� | en:NOT SUPPORT NOW
    char                strToolTip[MV_MAX_XML_DISC_STRLEN_C];

    enum MV_XML_Visibility   enVisivility;                       //�Ƿ�ɼ� | en:Visible
    enum MV_XML_AccessMode   enAccessMode;       // ch:����ģʽ | en:Access Mode
    int                 bIsLocked;          // ch:�Ƿ�������0-��1-��    Ŀǰ�ݲ�֧�� | en:Locked. 0-NO; 1-YES, NOT SUPPORT NOW
...
typedef struct _MV_XML_FEATURE_Enumeration_
{
    ...
    char                strCurrentSymbolic[MV_MAX_XML_SYMBOLIC_STRLEN_C];          // ch:��ǰSymbolic���� | en:Current Symbolic Index
